...
namespace PTI2_eAutosalloni.Controllers
{
    public class CarsController : Controller
    {
        ...
        [HttpGet]
        public async Task<IActionResult> Index(string search, string price)
        {
            ...
            if (!String.IsNullOrEmpty(search) || !String.IsNullOrEmpty(price))
            {
                punetor = punetor.Where(x => x.Model.Contains(search) || x.Price.ToString().Contains(price));
            }
